		...
		<div class="row">
			<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				...
				<tbody>
					...
					<tr>
						<td>내용</td>
						<td colspan="2" style="height:300px; text-align: Left;"><%=trade.getTradeContent().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt").replaceAll("\n", "<br>")%></td>
					</tr>
				</tbody>
			</table>
			...
			<div class="container">
				<div class="row">
					 <table class="table table-striped" style="text-align: center; border: 1px solid #dddddd; max-height: 500px; overflow-y: auto;">
						<thead>
						</thead>
						<tbody>
							<%
								CommentDAO commentDAO = new CommentDAO();
								ArrayList<Comment> list = commentDAO.getList(pageNumber, tradeID);
								for(int i=0;i<list.size();i++) {
							%>	
							<tr>
			                    <td style="width: 15%; border-right: 1px solid #dddddd;"><%= list.get(i).getUserID() %></td>
			                    <td style="width: 70%; border-bottom: 1px solid #dddddd;"><%= list.get(i).getTradeComment().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt").replaceAll("\n", "<br>") %></td>
			                    <td style="width: 15%; border-right: 1px solid #dddddd;"><%= list.get(i).getCommentDate() %></td>
			                </tr>
							<% 
								}
							%>				
			...
			<div class="container">
				...
				<form method="post" action="commentAction.jsp">
					<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
						<tbody>
							<tr>
								<td><input type="text" class="form-control" placeholder="댓글" name="tradeComment" maxlength="100"></td>
							</tr>
						</tbody>
					</table>
			...
			<div style="text-align: left; width: 100%;">
				...
				%>
						<a href="updateTrade.jsp?tradeID=<%= tradeID %>" class="btn btn-primary">수정</a>				
						<a onclick="return confirm('정말로 삭제하시겠습니까?')" href="deleteAction.jsp?tradeID=<%= tradeID %>" class="btn btn-primary">삭제</a>
				<%		
					}
				%>	
        	</div>
